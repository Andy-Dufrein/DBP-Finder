...
def make_folds(
    ...
    y = df["label"]
    groups = df["cluster"]
    sgkf = StratifiedGroupKFold(n_splits==n_splits if n_splits > 1 else 5,
...
def make_stratified_folds(
    df: pd.DataFrame, n_splits: int = 5, default_train_size: float = 0.8
) -> tuple[list[pd.DataFrame], list[pd.DataFrame]]:
    # Prepare data for GroupKFold
    X = df["sequence"]
    y = df["label"]
    ...
    if n_splits > 1:
        ...
        train, valid = train_test_split(df, train_size=default_train_size, stratify=y)
        train_folds.append(train)
        valid_folds.append(valid)
    return train_folds, valid_folds
